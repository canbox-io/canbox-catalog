/**
 * Action 2：校验与刷新（Gitee 实例，分片轮询）
 * 检查仓库可访问性、检测改名重定向、刷新元数据、重新校验 .canbox-app / package.json、计算评分
 * 每次处理一个分片
 *
 * 平台差异：Gitee 用 GET /v5/repos/{owner}/{repo} 判断可访问性（404=gone、403/401=private），
 * 文件内容走 raw（需显式分支，取仓库 default_branch）。
 */
...
if (argPlatform !== PLATFORM) {
    console.error(`[verify] This entry only supports platform "${PLATFORM}", got "${argPlatform}"`);
    process.exit(1);
}
...
async function verifyApp(app) {
    ...
    // 2. 检测改名 / 组织迁移（owner/repo 变化）
    if (repoData) {
        const actualOwner = repoData.owner ? repoData.owner.login : null;
        const actualRepo = repoData.name;
        if (actualOwner && actualRepo && (actualOwner !== owner || actualRepo !== repoName)) {
            console.log(`[verify] Repository moved: ${owner}/${repoName} → ${actualOwner}/${actualRepo}`);
    ...
    // 4. 重新校验 .canbox-app 和 package.json（Gitee raw 需显式分支）
    const actualRepoMatch = app.repo.match(ADAPTER.hostPattern);
    const currentOwner = actualRepoMatch ? actualRepoMatch[1] : owner;
    const currentRepoName = actualRepoMatch ? actualRepoMatch[2].replace(/\.git$/, '') : repoName;
    ...
    try {
        canboxApp = await helpers.fetchRepoFile(currentOwner, currentRepoName, helpers.CANBOX_APP_FILENAME, branch);
    ...
    try {
        pkg = await helpers.fetchRepoFile(currentOwner, currentRepoName, 'package.json', branch);
